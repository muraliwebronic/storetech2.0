...
export default function ProductConfigurator() {
  // State
  const { value: storedModules, set: setStoredModules } = useLocalStorage<
    string[]
  >("selectedModules", []);
  ...
  useEffect(() => {
    ...
    const merged: ModulesState = moduleKeys.reduce((acc, key) => {
      acc[key] = {
        ...initialModules[key],
        selected: storedModules.includes(key),
      };
      return acc;
    }, {} as ModulesState);
    setModules(merged);
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-4 space-y-12">
            ...
            {/* Sticky Action Area */}
            <div className="sticky top-24 bg-black text-white p-8 rounded-[2rem] shadow-2xl">
...
function ConfigFormModal({
  ...
  const [api] = useState(
    "https://wpdemo.webronics.com/wp-json/webronic/v1/contact"
  );
  const [noti, contextHolder] = notification.useNotification();

  const selectedItems = Object.values(modules)
    .filter((m) => m.selected)
    .map((m) => m.name);
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {contextHolder}
          {/* Backdrop - z-index increased to 100 to beat NavBar */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Modal - z-index increased to 100 */}
          <motion.div
          ...
          >
            {/* Increased max-width to 3xl for wider layout on MD screens */}
            <div className="bg-white w-full max-w-3xl rounded-[2rem] shadow-2xl overflow-hidden pointer-events-auto flex flex-col md:flex-row max-h-[90vh]">
              {/* LEFT SIDE: SUMMARY (Visible on MD+, scrollable if long) */}
              <div className="hidden md:block w-1/3 bg-gray-50 border-r border-gray-100 p-8 overflow-y-auto">
                ...
                <div className="space-y-3">
                  {selectedItems.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 text-sm font-bold text-black"
                    >
                      <div className="mt-0.5 w-4 h-4 rounded-full bg-green-500/10 text-green-600 flex items-center justify-center shrink-0">
              ...
              <div className="w-full md:w-2/3 flex flex-col">
                ...
                <div className="p-6 md:p-8 border-b border-gray-100 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Enter your details to receive a quote.
                    </p>
                  </div>
                  <button
                    onClick={onClose}
                ...
                <div className="p-6 md:p-8 overflow-y-auto">
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Summary Mobile Only */}
                    <div className="md:hidden bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedItems.map((item, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-white border border-gray-200 rounded-md text-xs font-bold"
                          >
                            {item}
                          </span>
                        ))}
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <div className="md:col-span-2">
                        <label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5 block ml-1">
                          Full Name
                        </label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full bg-white border border-gray-200 rounded-xl pl-11 pr-4 py-2.5 text-sm font-medium focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all"
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-black text-white font-bold py-3.5 rounded-xl hover:bg-gray-800 transition-all flex items-center justify-center gap-2 shadow-lg shadow-black/20"
                      >
                        {loading ? (
